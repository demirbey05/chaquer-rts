...
bytes32 constant _tableId = bytes32(abi.encodePacked(bytes16(""), bytes16("MineCaptureResul")));
bytes32 constant MineCaptureResultTableId = _tableId;

struct MineCaptureResultData {
  address winner;
  address loser;
  bool isDraw;
}

library MineCaptureResult {
  ...
  function getMetadata() internal pure returns (string memory, string[] memory) {
    ...
    return ("MineCaptureResult", _fieldNames);
  }
  ...
  function emitEphemeral(bytes32 key, MineCaptureResultData memory _table) internal {
  ...
  function emitEphemeral(IStore _store, bytes32 key, MineCaptureResultData memory _table) internal {
