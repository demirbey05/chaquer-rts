...
library GameMetaData {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "", name: "GameMetaData", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x7462000000000000000000000000000047616d654d6574614461746100000000);

  FieldLayout constant _fieldLayout =
    FieldLayout.wrap(0x00d70a0120010120142020202001000000000000000000000000000000000000);

  // Hex-encoded key schema of (uint256)
  Schema constant _keySchema = Schema.wrap(0x002001001f000000000000000000000000000000000000000000000000000000);
  // Hex-encoded value schema of (uint256, uint8, uint8, uint256, address, uint256, uint256, uint256, uint256, bool, string)
  Schema constant _valueSchema = Schema.wrap(0x00d70a011f00001f611f1f1f1f60c50000000000000000000000000000000000);
    ...
    )
  {
    ...
    state = State(uint8(Bytes.getBytes1(_blob, 32)));

    mapId = (uint8(Bytes.getBytes1(_blob, 33)));

    startBlock = (uint256(Bytes.getBytes32(_blob, 34)));

    winner = (address(Bytes.getBytes20(_blob, 66)));

    numberOfCastle = (uint256(Bytes.getBytes32(_blob, 86)));

    colorCursor = (uint256(Bytes.getBytes32(_blob, 118)));

    numberOfPlayer = (uint256(Bytes.getBytes32(_blob, 150)));

    limitOfPlayer = (uint256(Bytes.getBytes32(_blob, 182)));

    isInited = (_toBool(uint8(Bytes.getBytes1(_blob, 214))));
  }
