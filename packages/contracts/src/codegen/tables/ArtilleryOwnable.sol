...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_TABLE, bytes14(""), bytes16("ArtilleryOwnable")))
);
ResourceId constant ArtilleryOwnableTableId = _tableId;

FieldLayout constant _fieldLayout = FieldLayout.wrap(
  0x0034020014200000000000000000000000000000000000000000000000000000
);

struct ArtilleryOwnableData {
  address owner;
  uint256 gameID;
}

library ArtilleryOwnable {
  /**
   ...
   */
  function get(bytes32 key) internal view returns (ArtilleryOwnableData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
   ...
   * @notice Get the full data.
   */
  function _get(bytes32 key) internal view returns (ArtilleryOwnableData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = key;
   ...
   */
  function set(bytes32 key, ArtilleryOwnableData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.owner, _table.gameID);
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(bytes32 key, ArtilleryOwnableData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.owner, _table.gameID);
  ...
  function decode(
    ...
    PackedCounter,
    bytes memory
  ) internal pure returns (ArtilleryOwnableData memory _table) {
    (_table.owner, _table.gameID) = decodeStatic(_staticData);
  }
