...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_TABLE, bytes14(""), bytes16("ResourceInited")))
);
ResourceId constant ResourceInitedTableId = _tableId;
...
library ResourceInited {
   ...
   * @notice Get isInited.
   */
  function _get(uint256 gameID) internal view returns (bool isInited) {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(uint256(gameID));
  ...
  function setIsInited(uint256 gameID, bool isInited) internal {
    ...
    StoreSwitch.setStaticField(_tableId, _keyTuple, 0, abi.encodePacked((isInited)), _fieldLayout);
  }
   ...
   * @notice Set isInited.
   */
  function _set(uint256 gameID, bool isInited) internal {
    bytes32[] memory _keyTuple = new bytes32[](1);
    _keyTuple[0] = bytes32(uint256(gameID));
   ...
   */
  function encodeStatic(bool isInited) internal pure returns (bytes memory) {
   ...
   */
  function encode(bool isInited) internal pure returns (bytes memory, PackedCounter, bytes memory) {
    bytes memory _staticData = encodeStatic(isInited);
