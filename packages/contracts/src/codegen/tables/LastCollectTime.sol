...
ResourceId constant _tableId = ResourceId.wrap(
  bytes32(abi.encodePacked(RESOURCE_TABLE, bytes14(""), bytes16("LastCollectTime")))
);
ResourceId constant LastCollectTimeTableId = _tableId;
...
library LastCollectTime {
   ...
   * @notice Get lastCollect.
   */
  function _get(address owner, uint256 gameID) internal view returns (uint256 lastCollect) {
    bytes32[] memory _keyTuple = new bytes32[](2);
    _keyTuple[0] = bytes32(uint256(uint160(owner)));
    _keyTuple[1] = bytes32(uint256(gameID));
  ...
  function setLastCollect(address owner, uint256 gameID, uint256 lastCollect) internal {
    ...
    StoreSwitch.setStaticField(_tableId, _keyTuple, 0, abi.encodePacked((lastCollect)), _fieldLayout);
  }
   ...
   * @notice Set lastCollect.
   */
  function _set(address owner, uint256 gameID, uint256 lastCollect) internal {
    bytes32[] memory _keyTuple = new bytes32[](2);
    _keyTuple[0] = bytes32(uint256(uint160(owner)));
    _keyTuple[1] = bytes32(uint256(gameID));
   ...
   */
  function encodeStatic(uint256 lastCollect) internal pure returns (bytes memory) {
   ...
   */
  function encode(uint256 lastCollect) internal pure returns (bytes memory, PackedCounter, bytes memory) {
    bytes memory _staticData = encodeStatic(lastCollect);
