...
library ColorOwnable {
   ...
   */
  function set(bytes32 key, uint256 colorIndex, uint256 gameID) internal {
   ...
   * @notice Set the full data using individual values.
   */
  function _set(bytes32 key, uint256 colorIndex, uint256 gameID) internal {
    bytes memory _staticData = encodeStatic(colorIndex, gameID);
   ...
   */
  function decodeStatic(bytes memory _blob) internal pure returns (uint256 colorIndex, uint256 gameID) {
  ...
  function decode(
    ...
    (_table.colorIndex, _table.gameID) = decodeStatic(_staticData);
  }
   ...
   */
  function encodeStatic(uint256 colorIndex, uint256 gameID) internal pure returns (bytes memory) {
    return abi.encodePacked(colorIndex, gameID);
  }
