...
bytes32 constant _tableId = bytes32(abi.encodePacked(bytes16(""), bytes16("ColorOwnable")));
bytes32 constant ColorOwnableTableId = _tableId;
...
library ColorOwnable {
  ...
  function set(bytes32 key, uint256 colorIndex, uint256 gameID) internal {
    bytes memory _data = encode(colorIndex, gameID);
  ...
  function set(bytes32 key, ColorOwnableData memory _table) internal {
    set(key, _table.colorIndex, _table.gameID);
  }

  /** Set the full data using the data struct (using the specified store) */
  function set(IStore _store, bytes32 key, ColorOwnableData memory _table) internal {
    set(_store, key, _table.colorIndex, _table.gameID);
  }

  /** Decode the tightly packed blob using this table's schema */
  function decode(bytes memory _blob) internal pure returns (ColorOwnableData memory _table) {
    _table.colorIndex = (uint256(Bytes.slice32(_blob, 0)));

    _table.gameID = (uint256(Bytes.slice32(_blob, 32)));
  }

  /** Tightly pack full data using this table's schema */
  function encode(uint256 colorIndex, uint256 gameID) internal pure returns (bytes memory) {
