...
bytes32 constant _tableId = bytes32(abi.encodePacked(bytes16(""), bytes16("NavalWarResult")));
bytes32 constant NavalWarResultTableId = _tableId;

struct NavalWarResultData {
  address winner;
  address loser;
  bool isDraw;
}

library NavalWarResult {
  ...
  function emitEphemeral(bytes32 key, NavalWarResultData memory _table) internal {
  ...
  function emitEphemeral(IStore _store, bytes32 key, NavalWarResultData memory _table) internal {
