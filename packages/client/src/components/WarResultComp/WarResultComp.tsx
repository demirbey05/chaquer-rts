    ...
    return (
        <div className="war-result-bar">
            ...
            {warResults && warResults.data.map((data, key) => {
                if (data?.isDraw === true) {
                    return <p className='text-white p-4 bg-primary' key={key}>No body has won the war. Draw!</p>
                }
                else if (data?.winner === userWallet?.address.toLocaleLowerCase()) {
                    return <p className='text-white p-4 bg-success' key={key}>You have won the war. Win!</p>
                }
                else if (data?.loser === userWallet?.address.toLocaleLowerCase()) {
                    return <p className='text-white p-4 bg-danger' key={key}>You have lost the war. Lose!</p>
                }
            })}
        </div>
    )
