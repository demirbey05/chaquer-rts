import { Entity,HasValue,getComponentValueStrict, runQuery } from "@latticexyz/recs";


export function findCastleCloseArmies(castleID:Entity,Position:any,CastleOwnable:any,ArmyOwnable:any,ArmyConfig:any){
    ...
    allArmies.forEach((element)=>{
        ...
        const distance = Math.abs((positionOfArmy.x as number)-(castlePosition.x as number)) + Math.abs((positionOfArmy.y as number)-(castlePosition.y as number))
